...
/**
 * Holds the content of a single log entry regarding parser messages.
 * A log entry indicates if a line has been rejected by parser. This can be used
 * to check if a file was parsed without any significant errors.
 * {@link Throwable}s can also be collected by log entries.
 */
public class ParserLogEntry {
    ...
    /**
     * Indicates if the line had to be rejected.
     * Evaluate all log entries for this flag to check if a file contained any
     * significant errors.
     * <p>
     * Note that multiple log entries could refer to the same line and this
     * flag only refers to the current message. An indication of false (not
     * rejected) could still mean that another log entry exists for the same
     * line which caused the line to actually have been rejected.
     * </p>
     * @return Has the line been rejected by parser? (true = rejected; if false, you still may want to check other entries)
     */
    ...
    /**
     * Returns the associated {@link Throwable} if an exception has occured
     * during parsing.
     * Just holding a {@link Throwable} does not indicate a fatal error which
     * resulted in rejection of a line. Instead, check {@link #isLineRejected()}
     * if you want to know if parsing was able to continue.
